...
export const ProjectCard = ({ title, category, images, color = "bg-yellow-300", summary, techStack = [], links = [] }) => {
  return (
    <div
      className="group relative h-[340px] sm:h-[400px] w-full bg-white border-2 border-black rounded-2xl overflow-hidden shadow-[6px_6px_0px_0px_rgba(0,0,0,1)] hover:shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] hover:translate-x-[4px] hover:translate-y-[4px] transition-all duration-300 select-none flex flex-col"
    >
      {/* Image */}
      <div className="w-full h-[170px] sm:h-[200px] p-2.5 shrink-0">
        <div className="relative w-full h-full rounded-xl overflow-hidden bg-gray-100 border border-black/20">
          <img
            src={images?.[0]}
            alt={title}
      ...
      {/* Content area */}
      <div className="flex-1 p-3 sm:p-4 overflow-y-auto min-h-0 flex flex-col">
        ...
        <div className="pt-16 pl-4 pr-4 space-y-2 flex-1 flex flex-col">
          {/* Title */}
          <h3 className="text-base sm:text-lg font-black text-black uppercase tracking-tight drop-shadow-sm leading-snug line-clamp-2">
            {title}
          </h3>

          {/* Summary */}
          {summary && (
            <p className="text-[11px] sm:text-xs text-gray-700 leading-relaxed line-clamp-3">
              {summary}
            </p>
          )}
          ...
          {techStack && techStack.length > 0 && (
            <div className="flex flex-wrap gap-1.5">
              {techStack.slice(0, 4).map((tech, idx) => (
                <span
                  key={idx}
                  className="text-[10px] px-2 py-0.5 bg-gray-100 border border-black rounded font-bold text-gray-800 truncate max-w-[120px]"
                  title={tech}
                >
                  {tech}
                </span>
              ...
              {techStack.length > 4 && (
                <span className="text-[10px] px-2 py-0.5 bg-black text-white border border-black rounded font-bold">
                  +{techStack.length - 4}
                </span>
              )}
            </div>
            ...
            <div className="flex flex-wrap gap-2 mt-auto pt-2">
              {links.map((link, idx) => (
                <a
                  ...
                  target="_blank"
                  rel="noreferrer"
                  className={`inline-flex items-center gap-1 px-3 py-1 text-[10px] font-black uppercase border-2 border-black rounded-lg shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] hover:shadow-none hover:translate-x-[2px] hover:translate-y-[2px] transition-all ${link.className || 'bg-yellow-300 text-black hover:bg-yellow-400'}`}
                >
                  {link.icon}
                  {link.text}
                </a>
              ...
              <div className="flex gap-2">
                {data.links && data.links.map((link, idx) => (
                  <a
                    ...
                    target="_blank"
                    rel="noreferrer"
                    className={`inline-flex items-center gap-1.5 bg-yellow-300 border-2 border-black px-4 py-1.5 rounded font-black uppercase text-xs shadow-[3px_3px_0px_0px_rgba(0,0,0,1)] hover:translate-x-[1px] hover:translate-y-[1px] hover:shadow-none hover:bg-yellow-400 transition-all ml-auto ${link.className || ''}`}
                  >
                    {link.icon}
                    {link.text}
                  </a>
